...
#scene.camera.pos = -D*s0        #investigate this and alternatives!HERE!!!!HERE!
if s0.x == -1:
    scene.lights=[distant_light(direction=vector(0.22, 0.44, 0.88),
            color=color.gray(0.9)),
            distant_light(direction=vector(-0.88, -0.22, -0.44),
            color=color.gray(0.9))]
...
for p in params:
    ...
    if opac > 0.2:
        ...
        if falseColor:
            ...
            beams.append(sphere(pos=L*p[0],radius=beam_rad,
                color=color.white))
