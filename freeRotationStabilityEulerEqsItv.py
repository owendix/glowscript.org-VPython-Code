...
use_quat = True     #True is best

show_axes = False    #attach visible axes to the book
leave_trail = False
prt_check = False
"""
...
laxis = 1.25*a
clraxis = color.blue
neon_green = vec(0.223,1,0.078)
clrtip = [color.red,color.blue,neon_green]
...
slider(min=the_rate0/10,value=the_rate0,max=the_rate0*10,length=canv_width,bind=set_rate)
...
for s_ax in spin_axis:
    ...
    if scount > 1:
        book.visible=False
        if show_axes:
            ax3cyl.visible=False
            ax3tip.visible=False
            ax3tip.make_trail=False
            ax1cyl.visible=False
            #no tip on ax1
    if 'a' in s_ax:
        print('a -->3, b -->1, c-->2')
        ...
        print('c-->3, a-->1, b-->2')
        if show_axes and leave_trail:
            print('Slight variation in energy means trail should not close')
    ...
    if use_quat:
        ...
        if show_axes:
            #use xyz coordinates
            ax3cyl = cylinder(pos=vec(0,0,0),axis=laxis*hat(book.axis),radius=raxis,color=clraxis)
            ax1cyl = cylinder(pos=vec(0,0,0),axis=laxis*hat(book.up),radius=raxis,color=clraxis)        
            ax3tip = sphere(pos=ax3cyl.axis,radius=0.5*raxis,color=clrtip[scount-1],
                make_trail=False,retain=2000,trail_radius=0.5*raxis)
    ...
    if solveNumerically:
        while t < t_stop:
            ...
            if use_quat:
                ...
                if show_axes:
                    ax3cyl.axis=1.5*a*hat(book.axis)
                    ax1cyl.axis=1.5*a*hat(book.up)
                    ax3tip.pos=ax3cyl.axis
                    if t == 0:
                        ax3tip.make_trail=leave_trail
            else:
