...
instructions = label(text='To navigate: left, up, right; To shoot: spacebar',
    pos=vec(0,-0.8*L,0),box=False,line=False,opacity=0,height=18,
...
def make_worlds():
    ...
    for n in range(Nworlds):
        ...
        rvec = rvec*(L-world_r)
        if mag(rvec) < world_r:
            rvec = rvec + (4*r_L+world_r)*hat(rvec) #don't put it in the center
...
def make_world_nextround():
    ...
    while rvec.x==0 and rvec.y == 0:
        rvec = vec.random()
    speed = 0.9*v_max*abs(rvec.z) + 0.1*v_max  #max as fast as you, min 10%
    rvec.z = 0
...
def check_target_shot():
    global worlds, bullets, lose, field_clear
    for b in bullets:
        if b.visible:
            #check if I got shot
