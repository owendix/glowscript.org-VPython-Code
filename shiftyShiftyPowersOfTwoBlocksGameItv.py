...
font_h = 26 #24, good for all values
canv_w = 500
canv_h = canv_w
scn_range = 3
...
nclr = 6            #need 14 different colors at least (nclr*2)
d_color=(750-380)/nclr
for s in [vec(0.5,0.5,0.5),vec(0.25,0.25,0.25),vec(0,0,0)]:
    ...
    for i in range(0,4):
        for j in range(0,4):
            ...
            squares[i][j].num = label(text=str(2**0),pos=vec(i,j,0),height=font_h,
                ...
                color=color.gray(0.0),visible=False)
            #squares[i][j].num = text(text=str(2**0),pos=vec(i,j,0),height=0.3,
            #    font='sans',color=color.white,visible=False)
            ...
            for j in range(0,4):
                ...
                for i in shift_idx:
                    if squares[i][j].add == 1:
                        ...
                        if the_exp >= 14:
                            squares[i][j].num.height=int(font_h*(9/13))
                        else:
                            squares[i][j].num.height=font_h
