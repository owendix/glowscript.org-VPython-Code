...
Supposed to demonstrate that spinning around the intermediate principal axis 
is unstable

This solves with both the analytical and numerical methods. In both cases, 
...
numerical integration technique and the odd unstable behavior with the 
analytical method due to the poor approximation that the linearized form 
of Euler's equations makes in the long term behavior. The exponential that 
...
#Initial conditions
spin_axis = 'a'       #'a', 'b', or 'c': primary spin axis, all perturbed

solveNumerically = True   #False not set up yet
print('Solving analytically? ',not solveNumerically)

which_method = 2        #0,1,2
methods = ['mixedomega','mixedangle','simult']
method = methods[which_method]
...
if solveNumerically:
    if 'mixedomega' in method:
        while True:
            #Numerically integrate: mixed midpoint. Doesn't simultaneously update
            rate(the_rate)
            
            #These are measured in the 1,2,3 basis coordinates
            #Need to change bases. Define Euler angles? That is one way.
            #To change up = height = 2 axis, rotate around z-axis by alpha
            #If I use dot products, I need to make sure height has unit length
            #Definitions of euler angles
            #will depend on my initial setup, which I should consider
            #refining for ease.
            #Also, I'm not sure acos has the domain I want/need
    ...
    elif 'mixedangle' in method:
        while True:
            #Numerically integrate: updates omegas simultaneously,
            #but uses future omegas for current angle update
            rate(the_rate)
            ...
            tmp1 = w1 - w2*w3*I321*2*dt
            tmp2 = w2 - w3*w1*I132*2*dt
            tmp3 = w3 - w1*w2*I213*2*dt
    ...
    Ifrac1 = I132*I321
    #Ifrac2 = I321/I132      #if one fraction < 0, both are
    t = 0
