...
scn = canvas(width=canvw, height=canvh, forward=the_forward,
...
fish.append(pyramid(pos=fish[0].pos+f_dtail, axis=vec(-f_hw,0,0), 
                width=f_hw, height=f_hw, up=vec(0,0,f_hw),color=fish_color))
        #[body,tail]
...
instructs = label(text='Drag person around to see fish\'s image size and location\n'+
            'Press spacebar to see from person\'s eyes\t\t\t\t\t\t\t\t\n'+
            'Lake bottom edges are unadjusted\t\t\t\t\t\t\t\t\t\t\t\t',
            pos=fish[0].nose + vec(-2*f_L,1.5*f_hw,0), height=18, box=False, line=False,
...
def make_image(is_initial):
    ...
    if is_initial:
        ...
        image.append(pyramid(pos=image[0].pos+img_dtail, axis=img_tailax, 
                    width=img_w, height=img_h, up=vec(0,0,img_h),
                    color=img_color,opacity=img_opac))
